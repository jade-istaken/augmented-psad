...
class HistogramMemoryBank(nn.Module):
    ...
    def __init__(self, num_classes: int):
        ...
        self.register_buffer("max_train_distance",
                             torch.tensor(0.0))  # used for the adaptive scaling of scores later on
    ...
    def build(self, masks: torch.Tensor):
        ...
        for mask in masks:
            counts = torch.bincount(mask.flatten(),
                                    minlength=self.num_classes)  # the minlength is so that all tensors are the same length even if the mask lacks certain classes
            proportions = counts / (
                        counts.sum() + 1e-8)  # normalizing the proportions of classes so that it adds up to 1.0
            histograms.append(proportions)

        self.memory_bank = torch.stack(histograms,
                                       dim=0)  # stack all the histograms into a new tensor which acts as the memory bank
    ...
    def _compute_adaptive_scaling(self):
        ...
        for i in range(len(self.memory_bank)):
            ...
            distances = torch.cdist(test_hist, self.memory_bank,
                                    p=2.0)  # get the euclidean distances between the candidate histogram and all other histograms
        ...
        self.max_train_distance = torch.tensor(
            max(max_dist, 1e-8))  # add a lower bound so that there isn't divide by zero when it comes to scaling

    def score(self, test_mask: torch.Tensor) -> Tuple[float, float]:
        ...
        distances = torch.cdist(test_proportions, memory_bank,
                                p=2.0)  # expected shape is [1, N] where N is number of histograms
